...
public class UpdateVersionUtil {
    ...
    public void downloadBackground(String downloadUrl) {
        ...
        if(SDCardUtils.getFreeSpace(ConstUtils.MemoryUnit.MB)>fileSize){
            ...
            // 内部存储
            request.setDestinationInExternalFilesDir(mContext, ConfigConstant.DOWNLOAD_PATH, apkName);
        }else{
     ...
     */
    public void installApk(String path, final Context context) {
        final File apkfile = new File(path);
        if (!apkfile.exists()) {
            return;
        }
        // 通过Intent安装APK文件
        Log.i("tmd", "installApk: +++++++++++++++++++++++++++++++");
         Intent i = new Intent(Intent.ACTION_VIEW);
         if (Build.VERSION.SDK_INT>= Build.VERSION_CODES.N){//7.0
            Uri contentUri = FileProvider.getUriForFile(context, "com.a.fileProvider", apkfile);
