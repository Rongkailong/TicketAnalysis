...
public class CodeForcastActivity extends BaseActivity<TicketOpenInfoPresenter> implements TicketOpenInfoContact.IView {
    private String code;
    ...
    private void initAdapter() {
       ...
       adapter=new CommonAdapter<QueryMutiOpenInfoModel.ShowapiResBodyBean.ResultBean>(this,R.layout.item_open_info) {
           ...
           protected void convert(ViewHolder holder, QueryMutiOpenInfoModel.ShowapiResBodyBean.ResultBean resultBean, int position) {
               ...
               if (position==0){
                   holder.setText(R.id.tv, resultBean.getExpect() );
               }else {
    ...
    public void setTicketOpenInfo(QueryMutiOpenInfoModel queryMutiOpenInfoModel) {
        if (queryMutiOpenInfoModel.getShowapi_res_body()==null||queryMutiOpenInfoModel.getShowapi_res_body().getResult()==null||queryMutiOpenInfoModel.getShowapi_res_body().getResult().get(0).getName()==null){
            ...
            QueryMutiOpenInfoModel.ShowapiResBodyBean.ResultBean resultBean=new QueryMutiOpenInfoModel.ShowapiResBodyBean.ResultBean();
            resultBean.setOpenCode(queryMutiOpenInfoModel.getShowapi_res_body().getResult().get(5).getOpenCode());
            resultBean.setExpect("预测结果为：");
            queryMutiOpenInfoModel.getShowapi_res_body().getResult().add(0, resultBean);
